...
class Model(nn.Module):
    def __init__(self, config, load_emb=False, path=None, bias=True, total_tokens=63, depth=5, top_k=8, threshold=1.0):
        ...
        if load_emb and not hasattr(config, "target_hidden_size"):
            ...
            import json
            try:
                if 'llama' in path or 'Llama' in path or 'LLaMa' in path or 'Qwen3' in path or 'QwQ' in path:
    ...
    def topK_genrate(self, hidden_states, input_ids, head, logits_processor):
        ...
        if top_k==1:
            draft_probs.append(last_headout.softmax(dim=-1))
    ...
    def autogressive_generate(
        ...
        if hasattr(self, "stable_kv") and self.stable_kv is not None:
            kv_len = self.stable_kv[0][0].size(2)
        ...
        if logits_processor is not None:
            logits = logits_processor(None, logits)[0]
            draft_probs.append(torch.softmax(logits, dim=-1).view(1,-1))
            next_token = torch.multinomial(draft_probs[-1], 1).view(1,-1)
        else:
            next_token = torch.argmax(logits, dim=-1, keepdim=True).view(1,-1)  # (1, 1)
