...
from linearAttention.lightningAttention import lightning_attn2_no_decay

def is_lower_triangular_mask(mask):
    if mask.dim() != 2 or mask.size(0) != mask.size(1):
        return False
    n = mask.size(0)
    lower_tri = torch.tril(torch.ones(n, n)).to(mask.device)
    return torch.equal(mask, lower_tri)
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # total_dim = self.q_dim + self.kv_dim + self.kv_dim + self.q_dim 
        ...
        # self.qkvu_proj = nn.Linear(self.hidden_size * 2, total_dim, bias=False)
    ...
    def _kv_product(self, k, v):
        """
        k: (bs, n_kv_h, L, d)   v: 同形
        返回 kv_prod: (bs, n_h, d, d)
        """
        return torch.matmul(k.transpose(-1, -2), v) 
    ...
    def forward(
        ...
        u = self.u_proj(hidden_states)
            # qkvu = self.qkvu_proj(hidden_states)
        # query_states, key_states, value_states, u = qkvu.split([self.q_dim, self.kv_dim, self.kv_dim, self.q_dim], dim=-1)

        query_states = query_states.view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
        key_states = key_states.view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)
        value_states = value_states.view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)

        query_states = self.silu(query_states)
        key_states = self.silu(key_states)
        ...
        kv_cur  = self._kv_product(key_states, value_states)
        cur_len = key_states.size(2)
        if past_key_value is None:
            kv_prev = None
            prev_len = 0
        else:
            kv_prev, prev_len = past_key_value
        ...
        if kv_prev is None:
            ...
            attn_output = lightning_attn2_no_decay(query_states, key_rep, value_rep)
            kv_total = kv_cur
            total_len = cur_len
        else:
            kv_total = kv_prev + kv_cur
            total_len = prev_len + cur_len
            if q_len == 1:
                kv_total_rep = repeat_kv(kv_total, self.num_key_value_groups)
                attn_output = torch.matmul(query_states, kv_total_rep) 
            else:
                kv_prev_rep = repeat_kv(kv_prev,self.num_key_value_groups)
                attn_hist = torch.matmul(query_states, kv_prev_rep)
                k_rep = repeat_kv(key_states,  self.num_key_value_groups)
                v_rep = repeat_kv(value_states,        self.num_key_value_groups)
                attn_w = torch.matmul(
                    query_states, k_rep.transpose(-1, -2)
                )
                # tril = torch.tril(
                #     torch.ones(q_len, q_len, device=attn_w.device, dtype=attn_w.dtype)
                # )
                attn_w = torch.tril(attn_w)
                attn_cur = torch.matmul(attn_w, v_rep)
                attn_output = attn_hist + attn_cur

        present_key_value = (kv_total, total_len) if use_cache else None
...
class LlamaDecoderLayeremb(nn.Module):
    ...
    def forward(
            ...
            attention_mask: Optional[torch.Tensor] = None,
            position_ids: Optional[torch.LongTensor] = None,
            past_key_value: Optional[List[Tuple[torch.Tensor,int]]] = None,
...
class Model(nn.Module):
    def __init__(self, config, load_emb=False, path=None, bias=True, total_tokens=63, depth=5, top_k=8, threshold=1.0):
        ...
        if load_emb and not hasattr(config, "target_hidden_size"):
            ...
            import json
            try:
                if 'llama' in path or 'Llama' in path or 'LLaMa' in path or 'Qwen3' in path:
    ...
    def forward(
            ...
            attention_mask: Optional[torch.Tensor] = None,
            position_ids: Optional[torch.LongTensor] = None,
            past_key_values: Optional[List[Tuple[torch.Tensor,int]]] = None,
    ...
    def autogressive_generate(
        ...
        if hasattr(self, "stable_kv") and self.stable_kv is not None:
            kv_len = self.stable_kv[0][1]    
            out_hidden, past_kv = self(
                hidden_states,
                input_ids = context_ids[:, kv_len:],
        ...
        self.stable_kv = past_kv            

        last_hidden = out_hidden[:, -1]                     # (1, h)
        logits = self.lm_head(self.norm(last_hidden))           # (1, vocab)
        if logits_processor is not None:
            logits = logits_processor(logits)
        next_token = torch.argmax(logits, dim=-1, keepdim=True)  # (1, 1)
        draft_probs.append(torch.softmax(logits, dim=-1))
        
        if self.config.vocab_size != self.config.draft_vocab_size:
            next_token = next_token + self.d2t[next_token]

        draft_tokens = [input_ids[:, -1], next_token[0]]

        cur_pos = context_ids.size(1)        
        for _ in range(depth - 1):
            ...
            cur_pos += 1
            last_hidden = out_hidden[:, -1]                 # (1,h)
            logits = self.lm_head(self.norm(last_hidden))           
            if logits_processor is not None:
                logits = logits_processor(logits)
            draft_probs.append(torch.softmax(logits, dim=-1))
        ...
        return torch.cat(draft_tokens, dim=-1).view(-1), draft_probs
